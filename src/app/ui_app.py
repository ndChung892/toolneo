...
from .team_flow import TeamFlow
from .team_train import TeamTrainer
from .template_loader import TemplateLoader
from .team_binding import TeamBinder
...
# Dark "auto111" palette (target screenshot).
C_SIDEBAR = "#20242b"          # header / tab strip
C_SIDEBAR_ACTIVE = "#39424e"
C_PANEL = "#2b3038"            # panels / group boxes
C_ROW_BG = "#262b33"          # table row (even)
C_ROW_ALT = "#2f353e"         # table row (odd)
C_ROW_SEL = "#3a7d44"         # selected row (green)
C_DANGER = "#e0453e"
C_DANGER_HOVER = "#c23a34"
C_TEXT = "#e6e9ef"            # primary light text on dark
C_TEXT_DIM = "#9aa4b2"
C_TAB_IDLE = "#2b3139"
C_TAB_ACTIVE = "#2f6fed"
C_TAB_ACTIVE_HOVER = "#2559c8"
C_ACCENT = "#2f6fed"
C_ACCENT_HOVER = "#2559c8"
C_GREEN = "#3aa856"
C_ORANGE = "#e08a1e"
C_BLUE = "#2f6fed"

# Hard-fixed window and section heights (px). Everything is pinned so clicking a
# tab or an action never reflows the layout.
APP_W = 1000
APP_H = 720
H_HEADER = 40
H_ACCOUNTS = 350
H_TABSTRIP = 36
H_TABCONTENT = APP_H - H_HEADER - H_ACCOUNTS - H_TABSTRIP  # remaining
...
class App(ctk.CTk):
    def __init__(self) -> None:
        super().__init__()
        self.title("VPT Account Management")
        self.geometry(f"{APP_W}x{APP_H}")
        self.minsize(820, 560)  # resizable again; just a sensible minimum
        ...
        self.team_binder = TeamBinder(self.cfg.load_team_config())
        self.team_binding_snapshot = self.team_binder.snapshot()
    ...
    def _show(self, name: str) -> None:
        """Compatibility shim: the sidebar is gone. 'accounts' is embedded in
        the main window; everything else opens as its own tool window."""
        if name in ("accounts", "bot"):
            return
        self._open_tool(name)

    # Tools that open in their own Toplevel window, with title and size.
    TOOL_META = {
        "train": ("Train 5 acc (3 map)", "560x520"),
        "auto_boss": ("Auto Boss", "900x600"),
        "auto_ttt": ("Auto TTT", "900x600"),
        "settings": ("Cài đặt chung", "480x360"),
        "logs": ("Logs", "760x480"),
        "new_tool": ("Tool mới", "520x400"),
    }

    # Bottom tabs (auto111 "Quản lý tài khoản" style). Each maps to a page that
    # applies to the accounts ticked in the table above.
    TABS = [
        ("daily", "Daily"),
    ...
    def _build_layout(self) -> None:
        # Resizable: header/tab-strip keep a fixed pixel height; the account
        # area and tab content take the remaining space via row weights, so the
        # window can be dragged bigger/smaller without breaking. Switching tabs
        # doesn't resize the window because its geometry is user-controlled.
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=3)   # account area
        self.grid_rowconfigure(3, weight=4)   # tab content
        self._tool_windows: dict[str, ctk.CTkToplevel] = {}
        self._tab_frames: dict[str, ctk.CTkFrame] = {}
        self._tab_buttons: dict[str, ctk.CTkButton] = {}
        self.acc_check_vars: dict[str, ctk.BooleanVar] = {}

        header = ctk.CTkFrame(self, height=H_HEADER, fg_color=C_SIDEBAR, corner_radius=0)
        header.grid(row=0, column=0, sticky="ew")
        header.grid_propagate(False)
        header.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(header, text="QUẢN LÝ TÀI KHOẢN GAME VUA PHÁP THUẬT",
                     font=ctk.CTkFont(size=13, weight="bold")).grid(
            row=0, column=0, sticky="w", padx=14, pady=6)
        ctk.CTkButton(header, text="⚙", width=34, height=28, corner_radius=6,
                      font=ctk.CTkFont(size=16), fg_color=C_TAB_IDLE,
                      hover_color=C_SIDEBAR_ACTIVE, text_color=C_TEXT,
                      command=lambda: self._open_tool("settings")).grid(
            row=0, column=1, sticky="e", padx=(0, 12), pady=6)

        # Account area (TOP): expands with the window.
        self.content = ctk.CTkFrame(self, corner_radius=0, fg_color="transparent")
        self.content.grid(row=1, column=0, sticky="nsew")
        self.content.grid_rowconfigure(0, weight=1)
        self.content.grid_columnconfigure(0, weight=1)
        self.pages = {"accounts": self._pg_accounts()}
        self.pages["accounts"].grid(row=0, column=0, sticky="nsew")

        # Tab strip.
        strip = ctk.CTkFrame(self, height=H_TABSTRIP, fg_color=C_SIDEBAR, corner_radius=0)
        strip.grid(row=2, column=0, sticky="ew")
        strip.grid_propagate(False)
        for key, label in self.TABS:
            btn = ctk.CTkButton(strip, text=label, height=28, width=96, corner_radius=6,
                                fg_color=C_TAB_IDLE, hover_color=C_SIDEBAR_ACTIVE,
                                text_color=C_TEXT,
                                command=lambda k=key: self._switch_tab(k))
            btn.pack(side="left", padx=3, pady=4)
            self._tab_buttons[key] = btn

        # Tab content host: expands with the window.
        self.tab_host = ctk.CTkFrame(self, corner_radius=0, fg_color="transparent")
        self.tab_host.grid(row=3, column=0, sticky="nsew")
        self.tab_host.grid_rowconfigure(0, weight=1)
        self.tab_host.grid_columnconfigure(0, weight=1)
        self._switch_tab("train")

    # Builders for each bottom tab; unfinished tabs show a placeholder.
    def _tab_builder(self, key):
        return {
            "train": self._pg_train,
            "settings_tab": self._pg_caidat,
            "sotay": self._pg_auto_boss,
            "autoclick": self._pg_auto_ttt,
        }.get(key, lambda k=key: self._pg_placeholder(k))

    # ------------------------------------------------------------- Cài đặt tab
    DUNGEONS = ["Mê Huyễn Động", "Kho Báu Đại Mạc", "Lục Tiên Cảnh",
                "Liệt Diễm Thâm Uyên", "Trở Lại Lang Huyết", "Quỷ Hút Máu",
                "Thế Giới Số", "Thám Hiểm"]
    LEVELS = [f"Cấp {i}" for i in range(1, 10)]

    def _pg_caidat(self) -> ctk.CTkFrame:
        frame = ctk.CTkFrame(self.content)
        frame.grid_columnconfigure(0, weight=1)
        frame.grid_columnconfigure(1, weight=1)
        frame.grid_rowconfigure(0, weight=1)
        self.caidat_vars = getattr(self, "caidat_vars", {})

        # Left: daily-task settings (label + dropdown + dropdown/button).
        left = ctk.CTkFrame(frame, fg_color="transparent")
        left.grid(row=0, column=0, sticky="nsew", padx=(10, 6), pady=8)
        rows = [
            ("Lựa chọn Mật bảo", self.LEVELS, ["Liệt Diễm", "Băng Phong", "Lôi Đình"], None),
            ("Trồng trang viên với", ["Lông thú", "Da thú", "Cỏ"], None, "Trồng"),
            ("Đổi năng nổ với", ["Da thú", "Lông thú", "Cỏ"], None, "Đổi"),
            ("Hái thuốc", self.LEVELS, None, "Hái"),
            ("Câu cá", self.LEVELS, None, "Câu"),
            ("Nông trường", self.LEVELS, None, "Trồng"),
        ]
        for i, (label, opts1, opts2, action) in enumerate(rows):
            ctk.CTkLabel(left, text=label, width=130, anchor="w").grid(row=i, column=0, sticky="w", pady=5)
            m1 = ctk.CTkOptionMenu(left, values=opts1, width=96, height=28)
            m1.grid(row=i, column=1, padx=4, pady=5)
            self.caidat_vars[f"{label}#1"] = m1
            if opts2:
                m2 = ctk.CTkOptionMenu(left, values=opts2, width=110, height=28)
                m2.grid(row=i, column=2, padx=4, pady=5)
                self.caidat_vars[f"{label}#2"] = m2
            elif action:
                ctk.CTkButton(left, text=action, width=64, height=28,
                              command=lambda l=label: self.logger.info("CAIDAT", f"{l} -> chạy")).grid(
                    row=i, column=2, padx=4, pady=5)

        # Right: "Cài đặt phụ bản" — ▶ + checkbox per dungeon.
        rightp = ctk.CTkFrame(frame, fg_color=C_PANEL, border_width=1, border_color=C_SIDEBAR_ACTIVE)
        rightp.grid(row=0, column=1, sticky="nsew", padx=(6, 10), pady=8)
        ctk.CTkLabel(rightp, text="Cài đặt phụ bản", font=ctk.CTkFont(size=12, weight="bold")).pack(
            anchor="w", padx=12, pady=(8, 4))
        self.dungeon_vars = getattr(self, "dungeon_vars", {})
        for name in self.DUNGEONS:
            row = ctk.CTkFrame(rightp, fg_color="transparent")
            row.pack(fill="x", padx=12, pady=2)
            ctk.CTkButton(row, text="▶", width=26, height=24, fg_color=C_GREEN,
                          hover_color="#248049",
                          command=lambda n=name: self.logger.info("PHUBAN", f"{n} -> chạy")).pack(side="left")
            var = self.dungeon_vars.get(name) or ctk.BooleanVar(value=False)
            self.dungeon_vars[name] = var
            ctk.CTkCheckBox(row, text=name, variable=var, fg_color=C_GREEN, hover_color=C_GREEN,
                            font=ctk.CTkFont(size=12)).pack(side="left", padx=8)
        return frame

    def _pg_placeholder(self, key) -> ctk.CTkFrame:
        frame = ctk.CTkFrame(self.content)
        label = dict(self.TABS).get(key, key)
        ctk.CTkLabel(frame, text=f"{label}", font=ctk.CTkFont(size=15, weight="bold")).pack(
            padx=20, pady=(20, 6))
        ctk.CTkLabel(frame, text="Chức năng này sẽ bổ sung sau.",
                     text_color=C_TEXT_DIM).pack(padx=20, pady=4)
        return frame

    def _switch_tab(self, key: str) -> None:
        if key not in self._tab_frames:
            # Each tab lives in a fixed-height, horizontally-scrollable holder:
            # wide pages slide to the right instead of stretching the window.
            holder = ctk.CTkScrollableFrame(self.tab_host, orientation="horizontal",
                                            fg_color="transparent")
            prev = self.content
            self.content = holder
            try:
                page = self._tab_builder(key)()
            finally:
                self.content = prev
            # Bordered panel so each tool area is visually framed.
            try:
                page.configure(border_width=1, border_color=C_SIDEBAR_ACTIVE, fg_color=C_PANEL)
            except Exception:
                pass
            page.pack(fill="both", expand=True, padx=6, pady=4)
            self._tab_frames[key] = holder
        for frame in self._tab_frames.values():
            frame.grid_forget()
        self._tab_frames[key].grid(row=0, column=0, sticky="nsew")
        for k, btn in self._tab_buttons.items():
            active = k == key
            btn.configure(fg_color=C_ACCENT if active else C_TAB_IDLE,
                          text_color="#ffffff" if active else C_TEXT)

    def _open_tool(self, name: str) -> None:
        """Open a tool in its own Toplevel; reuse (deiconify) if already built.

        Windows are withdrawn — not destroyed — on close so their widgets keep
        existing and the poll loop can safely keep refreshing their labels."""
        ...
        if win is not None and win.winfo_exists():
            ...
            win.focus()
            return
        title, geometry = self.TOOL_META.get(name, ("Tool", "600x400"))
        win = ctk.CTkToplevel(self)
        win.title(title)
        win.geometry(geometry)
        win.protocol("WM_DELETE_WINDOW", lambda w=win: w.withdraw())
        builders = {
            "train": self._pg_train,
            ...
            "logs": self._pg_logs,
            "new_tool": self._pg_new_tool,
        }
        builder = builders.get(name)
        if builder is None:
            return
        # Builders parent their root frame to self.content; retarget it to the
        # Toplevel for the duration of this build, then restore.
        prev = self.content
        self.content = win
        try:
            frame = builder()
    ...
    def _pg_new_tool(self) -> ctk.CTkFrame:
        frame = ctk.CTkFrame(self.content)
        ctk.CTkLabel(frame, text="Cửa sổ tool mới", font=ctk.CTkFont(size=16, weight="bold")).pack(
            padx=20, pady=(24, 8))
        ctk.CTkLabel(frame, text="Khung trống sẵn sàng để gắn công cụ mới.",
                     text_color=C_TEXT_DIM).pack(padx=20, pady=4)
        return frame

    # -------------------------------------------------------------- train 5 acc
    def _pg_train(self) -> ctk.CTkFrame:
        frame = ctk.CTkFrame(self.content)
        frame.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(frame, text="Train 5 acc — 3 map", font=ctk.CTkFont(size=17, weight="bold")).grid(
            row=0, column=0, sticky="w", padx=16, pady=(14, 4))
        ctk.CTkLabel(
            frame,
            text=("Đăng nhập song song 5 acc → bật Auto → mỗi map: đi cùng map, "
                  "lập nhóm (memory), key đi giữa NPC để gặp quái. Chỉ KEY di chuyển; "
                  "member theo nhóm. Tự khởi động lại cả 5 nếu kẹt/crash."),
            text_color=C_TEXT_DIM, wraplength=500, justify="left").grid(
            row=1, column=0, sticky="w", padx=16, pady=(0, 10))

        card = ctk.CTkFrame(frame, fg_color=C_PANEL)
        card.grid(row=2, column=0, sticky="ew", padx=16, pady=(0, 12))
        card.grid_columnconfigure(0, weight=1)
        self.btn_train = ctk.CTkButton(card, text="Bắt đầu train 5 acc", height=36,
                                       fg_color=C_ACCENT, hover_color=C_ACCENT_HOVER,
                                       command=self._toggle_train)
        self.btn_train.grid(row=0, column=0, sticky="ew", padx=12, pady=(12, 6))
        self.lbl_train_state = ctk.CTkLabel(card, text="Train: IDLE", text_color=C_TEXT_DIM,
                                            justify="left", wraplength=500, anchor="w")
        self.lbl_train_state.grid(row=1, column=0, sticky="ew", padx=12, pady=(0, 12))
        return frame
    ...
    def _pg_accounts(self) -> ctk.CTkFrame:
        # Dark auto111 layout: left = Thông tin + Nhật ký + Hội VPT group boxes;
        # right = saved-account gridview + colored action buttons.
        ...
        # ================= LEFT COLUMN =================
        left = ctk.CTkFrame(frame, width=262, fg_color="transparent")
        left.grid(row=0, column=0, sticky="nsew", padx=(10, 6), pady=8)
        ...
        def group(parent, title):
            box = ctk.CTkFrame(parent, fg_color=C_PANEL, border_width=1, border_color=C_SIDEBAR_ACTIVE)
            ctk.CTkLabel(box, text=title, text_color="#ff8a3d",
                         font=ctk.CTkFont(size=12, weight="bold")).pack(anchor="w", padx=10, pady=(6, 2))
            return box

        # ---- Thông tin ----
        info = group(left, "Thông tin")
        info.grid(row=0, column=0, sticky="ew", pady=(0, 6))
        gr = ctk.CTkFrame(info, fg_color="transparent")
        gr.pack(fill="x", padx=10, pady=(0, 8))
        gr.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(gr, text="Tên", anchor="w").grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(gr, text="Ver", anchor="w").grid(row=0, column=1, sticky="w", padx=(6, 0))
        self.ent_info_name = ctk.CTkEntry(gr, height=26)
        self.ent_info_name.grid(row=1, column=0, sticky="ew", pady=(0, 4))
        self.ent_info_ver = ctk.CTkEntry(gr, width=48, height=26)
        self.ent_info_ver.grid(row=1, column=1, padx=(6, 0), pady=(0, 4))
        ctk.CTkLabel(gr, text="Link", anchor="w").grid(row=2, column=0, sticky="w")
        ctk.CTkLabel(gr, text="FPS", anchor="w").grid(row=2, column=1, sticky="w", padx=(6, 0))
        self.ent_info_link = ctk.CTkEntry(gr, height=26)
        self.ent_info_link.grid(row=3, column=0, sticky="ew", pady=(0, 4))
        self.ent_info_fps = ctk.CTkEntry(gr, width=48, height=26)
        self.ent_info_fps.grid(row=3, column=1, padx=(6, 0), pady=(0, 4))
        cbrow = ctk.CTkFrame(gr, fg_color="transparent")
        cbrow.grid(row=4, column=0, columnspan=2, sticky="ew", pady=(2, 0))
        self.var_viethoa = ctk.BooleanVar(value=False)
        self.var_multitab = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(cbrow, text="Việt hóa", variable=self.var_viethoa, width=18,
                        checkbox_width=18, checkbox_height=18, fg_color=C_GREEN,
                        font=ctk.CTkFont(size=11)).pack(side="left")
        ctk.CTkCheckBox(cbrow, text="Mở trùng tab", variable=self.var_multitab, width=18,
                        checkbox_width=18, checkbox_height=18, fg_color=C_GREEN,
                        font=ctk.CTkFont(size=11)).pack(side="left", padx=(6, 0))
        self.ent_info_pos = ctk.CTkEntry(cbrow, width=40, height=24, placeholder_text="Vị trí")
        self.ent_info_pos.pack(side="right")
        act = ctk.CTkFrame(info, fg_color="transparent")
        act.pack(fill="x", padx=10, pady=(0, 8))
        ctk.CTkButton(act, text="THÊM", width=60, height=28, fg_color=C_GREEN, hover_color="#2f8a47",
                      command=self._info_add).pack(side="left", padx=(0, 4))
        ctk.CTkButton(act, text="CẬP NHẬT", width=90, height=28, fg_color=C_ORANGE, hover_color="#c2760f",
                      command=self._info_update).pack(side="left", padx=4)
        ctk.CTkButton(act, text="XÓA", width=54, height=28, fg_color=C_DANGER, hover_color=C_DANGER_HOVER,
                      command=self._info_delete).pack(side="left", padx=4)

        # ---- Nhật ký ----
        logbox = group(left, "Nhật ký")
        logbox.grid(row=1, column=0, sticky="nsew", pady=(0, 6))
        top = ctk.CTkFrame(logbox, fg_color="transparent")
        top.pack(fill="x", padx=10)
        ctk.CTkCheckBox(top, text="Hiện log tài khoản", variable=ctk.BooleanVar(value=True),
                        width=18, checkbox_width=18, checkbox_height=18, fg_color=C_GREEN,
                        font=ctk.CTkFont(size=11)).pack(side="left")
        ctk.CTkButton(top, text="Xóa Log", width=64, height=24, fg_color=C_SIDEBAR_ACTIVE,
                      command=self._clear_logs).pack(side="right")
        self.log_box = ctk.CTkTextbox(logbox, fg_color=C_ROW_BG, font=ctk.CTkFont(family="Consolas", size=10))
        self.log_box.pack(fill="both", expand=True, padx=10, pady=(4, 8))

        # ---- Hội VPT ----
        foot = group(left, "Hội VPT TQ")
        foot.grid(row=2, column=0, sticky="ew")
        ctk.CTkLabel(foot, text="Discord: discord.gg/VNdbjsFnq7", text_color=C_TEXT_DIM,
                     font=ctk.CTkFont(size=10)).pack(anchor="w", padx=10)
        ctk.CTkLabel(foot, text="Facebook: fb.com/mc.lezi.vietnam", text_color=C_TEXT_DIM,
                     font=ctk.CTkFont(size=10)).pack(anchor="w", padx=10, pady=(0, 8))
        self.lbl_login_all = ctk.CTkLabel(foot, text="", text_color=C_TEXT_DIM,
                                          font=ctk.CTkFont(size=9), wraplength=230, justify="left")
        self.lbl_login_all.pack(anchor="w", padx=10, pady=(0, 4))
        ...
        ctk.CTkLabel(right, text="Danh sách tài khoản đã lưu", text_color="#ff8a3d",
                     font=ctk.CTkFont(size=12, weight="bold")).grid(row=0, column=0, sticky="w", pady=(0, 2))

        tv_wrap = ctk.CTkFrame(right, fg_color="transparent")
        tv_wrap.grid(row=1, column=0, sticky="nsew", pady=(0, 6))
        tv_wrap.grid_rowconfigure(0, weight=1)
        tv_wrap.grid_columnconfigure(0, weight=1)
        cols = ("sel", "stt", "ten", "kenh", "nhom", "pid")
        self.acc_tree = ttk.Treeview(tv_wrap, columns=cols, show="headings",
                                     style="Dark.Treeview", selectmode="browse")
        for c, t, w, anchor, stretch in [
            ("sel", "☑", 40, "center", False), ("stt", "STT", 46, "center", False),
            ("ten", "TÊN", 150, "w", True), ("kenh", "KÊNH", 84, "center", False),
            ("nhom", "NHÓM AUTO", 104, "center", False), ("pid", "PID", 72, "center", False)]:
            self.acc_tree.heading(c, text=t,
                                  command=self._toggle_check_all if c == "sel" else (lambda: None))
            self.acc_tree.column(c, width=w, anchor=anchor, stretch=stretch)
        vsb = ttk.Scrollbar(tv_wrap, orient="vertical", command=self.acc_tree.yview)
        self.acc_tree.configure(yscrollcommand=vsb.set)
        self.acc_tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")
        self.acc_tree.bind("<Button-1>", self._on_tree_click)
        self.acc_check: dict[str, bool] = {}

        ctk.CTkLabel(right, text="Thao tác", text_color="#ff8a3d",
                     font=ctk.CTkFont(size=12, weight="bold")).grid(row=2, column=0, sticky="w")
        actrow = ctk.CTkFrame(right, fg_color="transparent")
        actrow.grid(row=3, column=0, sticky="ew", pady=(2, 2))
        for label, cmd, color in [
            ("VÀO GAME", self._act_login_checked, C_BLUE), ("VÀO ALL", self._act_login_all, C_GREEN),
            ("AUTO ALL", self._act_auto_all, C_DANGER), ("NGỪNG AUTO", self._act_stop_auto, C_ORANGE),
            ("DỪNG ALL", self._act_stop_all, C_DANGER)]:
            ctk.CTkButton(actrow, text=label, height=36, fg_color=color,
                          font=ctk.CTkFont(size=13, weight="bold"),
                          command=cmd).pack(side="left", expand=True, fill="x", padx=2)

        ops = ctk.CTkFrame(right, fg_color="transparent")
        ops.grid(row=4, column=0, sticky="ew", pady=(4, 0))
        for label, cmd in [
            ("Sắp xếp CS", self._arrange_windows), ("Ẩn (đỡ GPU)", self._hide_all_windows),
            ("Ẩn taskbar", self._minimize_all_windows), ("Hiện", self._restore_all_windows),
            ("Đóng CS", self._close_all_windows)]:
            ctk.CTkButton(ops, text=label, height=26, width=92, fg_color=C_TAB_IDLE,
                          hover_color=C_SIDEBAR_ACTIVE, text_color=C_TEXT,
                          command=cmd).pack(side="left", padx=2)
        self.lbl_winops = ctk.CTkLabel(ops, text="", text_color=C_TEXT_DIM, font=ctk.CTkFont(size=11))
        self.lbl_winops.pack(side="left", padx=6)
        return frame

    # --------------------------------------------------------- account form/table
    def _load_acc_form(self, acc: dict) -> None:
        self.selected_acc_id = acc["id"]
        for ent, val in [(self.ent_info_name, acc.get("name", "")),
                         (self.ent_info_link, acc.get("launch_link", "")),
                         (self.ent_info_ver, acc.get("ver", "")),
                         (self.ent_info_fps, acc.get("fps", "")),
                         (self.ent_info_pos, acc.get("login_position", 1))]:
            ent.delete(0, "end")
            ent.insert(0, str(val))
        if hasattr(self, "acc_tree") and self.acc_tree.exists(acc["id"]):
            self.acc_tree.selection_set(acc["id"])

    def _setup_tree_style(self) -> None:
        if getattr(self, "_tree_styled", False):
            return
        self._tree_styled = True
        style = ttk.Style()
        try:
            style.theme_use("clam")  # allows full dark recolor
        except Exception:
            pass
        style.configure("Dark.Treeview", background=C_ROW_BG, fieldbackground=C_ROW_BG,
                        foreground=C_TEXT, rowheight=28, borderwidth=0, font=("Segoe UI", 10))
        style.configure("Dark.Treeview.Heading", background=C_SIDEBAR_ACTIVE, foreground=C_TEXT,
                        relief="flat", font=("Segoe UI", 10, "bold"))
        style.map("Dark.Treeview", background=[("selected", C_ROW_SEL)],
                  foreground=[("selected", "#ffffff")])
        style.map("Dark.Treeview.Heading", background=[("active", C_SIDEBAR_ACTIVE)])

    def _on_tree_click(self, event) -> None:
        row = self.acc_tree.identify_row(event.y)
        col = self.acc_tree.identify_column(event.x)
        if not row:
            return
        if col == "#1":  # the ✔ column toggles the tick
            self.acc_check[row] = not self.acc_check.get(row, False)
            self.acc_tree.set(row, "sel", "☑" if self.acc_check[row] else "☐")
        else:
            acc = self._find_acc(row)
            if acc:
                self._load_acc_form(acc)

    def _info_add(self) -> None:
        name = self.ent_info_name.get().strip()
        if not name:
            messagebox.showwarning("Thêm", "Nhập Tên (cũng là title cửa sổ Flash).")
            return
        new_id = self._new_account_id()
        acc = {"id": new_id, "name": name,
               "launch_link": self.ent_info_link.get().strip(),
               "ver": self.ent_info_ver.get().strip(), "fps": self.ent_info_fps.get().strip(),
               "profile_file": f"configs/profiles/{new_id}.json", "enabled": True,
               "login_position": max(1, min(3, _safe_int(self.ent_info_pos.get(), 1)))}
        self.accounts_data.setdefault("accounts", []).append(acc)
        self.cfg.save_accounts(self.accounts_data)
        self.cfg.ensure_profile(acc)
        self.selected_acc_id = new_id
        self._refresh_acc_table()
        self.logger.info(new_id, f'Account added "{name}"')

    def _info_update(self) -> None:
        acc = self._find_acc(self.selected_acc_id)
        if not acc:
            messagebox.showwarning("Cập nhật", "Chọn một acc trong bảng trước.")
            return
        acc["name"] = self.ent_info_name.get().strip() or acc["name"]
        acc["launch_link"] = self.ent_info_link.get().strip()
        acc["ver"] = self.ent_info_ver.get().strip()
        acc["fps"] = self.ent_info_fps.get().strip()
        acc["login_position"] = max(1, min(3, _safe_int(self.ent_info_pos.get(), 1)))
        self.cfg.save_accounts(self.accounts_data)
        self.cfg.ensure_profile(acc)
        self._refresh_acc_table()
        self.logger.info(acc["id"], "Account updated")

    def _info_delete(self) -> None:
        targets = self._checked_accounts() or ([self._find_acc(self.selected_acc_id)]
                                               if self.selected_acc_id else [])
        targets = [t for t in targets if t]
        if not targets:
            messagebox.showwarning("Xóa", "Tick hoặc chọn acc để xóa.")
            return
        names = ", ".join(t["name"] for t in targets)
        if not messagebox.askyesno("Xóa", f"Xóa: {names}?"):
            return
        ids = {t["id"] for t in targets}
        self.accounts_data["accounts"] = [a for a in self.accounts_data.get("accounts", [])
                                          if a["id"] not in ids]
        self.cfg.save_accounts(self.accounts_data)
        if self.selected_acc_id in ids:
            self.selected_acc_id = None
        self._refresh_acc_table()

    def _checked_accounts(self) -> list[dict]:
        return [a for a in self.accounts_data.get("accounts", [])
                if self.acc_check.get(str(a["id"]))]

    def _toggle_check_all(self) -> None:
        accounts = self.accounts_data.get("accounts", [])
        turn_on = not all(self.acc_check.get(str(a["id"])) for a in accounts) if accounts else False
        for a in accounts:
            aid = str(a["id"])
            self.acc_check[aid] = turn_on
            if hasattr(self, "acc_tree") and self.acc_tree.exists(aid):
                self.acc_tree.set(aid, "sel", "☑" if turn_on else "☐")

    def _targets_or_all(self) -> list[dict]:
        return self._checked_accounts() or list(self.accounts_data.get("accounts", []))

    def _act_login_checked(self) -> None:
        for acc in (self._checked_accounts()
                    or ([self._find_acc(self.selected_acc_id)] if self.selected_acc_id else [])):
            if acc:
                threading.Thread(target=self._launch_game, args=(acc,), daemon=True).start()

    def _act_login_all(self) -> None:
        self._login_all_parallel()

    def _act_auto_all(self) -> None:
        for acc in self._targets_or_all():
            self._first_scan_infinite_auto(acc)

    def _act_stop_auto(self) -> None:
        for acc in self._targets_or_all():
            self._reset_infinite_auto(acc)

    def _act_stop_all(self) -> None:
        for acc in self.accounts_data.get("accounts", []):
            self._reset_infinite_auto(acc)
        try:
            ...
            pass
        self._win_status("đã dừng tất cả auto")

    # ---------------------------------------------------------- window controls
    def _all_account_hwnds(self) -> list[int]:
        """Live Flash hwnds for every configured account (matched by title)."""
        hwnds: list[int] = []
        for acc in self.accounts_data.get("accounts", []):
            win = self._account_flash(acc)
            if win and win.hwnd:
                hwnds.append(int(win.hwnd))
        return hwnds

    def _win_status(self, text: str) -> None:
        if hasattr(self, "lbl_winops"):
            self.lbl_winops.configure(text=text)

    def _arrange_windows(self) -> None:
        from . import window_ops
        n = window_ops.arrange_grid(self._all_account_hwnds())
        self._win_status(f"đã xếp {n} cửa sổ")

    def _close_all_windows(self) -> None:
        if not messagebox.askyesno("Đóng tất cả", "Đóng tất cả cửa sổ Flash của các acc?"):
            return
        from . import window_ops
        hwnds = self._all_account_hwnds()
        for h in hwnds:
            window_ops.close_window(h)
        self._win_status(f"đã đóng {len(hwnds)} cửa sổ")

    def _hide_all_windows(self) -> None:
        from . import window_ops
        hwnds = self._all_account_hwnds()
        for h in hwnds:
            window_ops.hide_window(h)
        self._win_status(f"đã ẩn {len(hwnds)} cửa sổ (đỡ GPU)")

    def _minimize_all_windows(self) -> None:
        from . import window_ops
        hwnds = self._all_account_hwnds()
        for h in hwnds:
            window_ops.minimize_window(h)
        self._win_status(f"đã thu nhỏ {len(hwnds)} cửa sổ")

    def _restore_all_windows(self) -> None:
        from . import window_ops
        hwnds = self._all_account_hwnds()
        for h in hwnds:
            window_ops.restore_window(h)
        self._win_status(f"đã hiện {len(hwnds)} cửa sổ")

    def _login_all_parallel(self) -> None:
        """Launch every configured account's Flash at once (small stagger)."""
        flash_exe = self.global_cfg.get("flash_exe", "")
        if not flash_exe or not Path(flash_exe).is_file():
            messagebox.showerror("Đăng nhập tất cả", "flash.exe chưa cấu hình hợp lệ (Cài đặt).")
            return
        accounts = [a for a in self.accounts_data.get("accounts", [])
                    if str(a.get("launch_link") or "").strip()]
        if not accounts:
            messagebox.showwarning("Đăng nhập tất cả", "Chưa có account nào có link.")
            return

        def launch(acc):
            self.single_auto_start.__class__(self.logger).start_account(flash_exe, acc, 150)

        for i, acc in enumerate(accounts):
            threading.Thread(target=launch, args=(acc,), daemon=True).start()
            if i < len(accounts) - 1:
                time.sleep(0.2)
        self._win_status(f"đang đăng nhập song song {len(accounts)} acc...")
    ...
    def _refresh_acc_table(self) -> None:
        ...
        # Poll iterates these dicts; keep them present but empty.
        self.infinite_auto_buttons = {}
        self.infinite_auto_next_buttons = {}
        self.infinite_auto_reset_buttons = {}

        # Repopulate the Treeview, preserving tick state and selection.
        self.acc_tree.delete(*self.acc_tree.get_children())
        for idx, acc in enumerate(accounts):
            aid = str(acc["id"])
            flash = self._account_flash(acc)
            pid = flash.pid if flash else "-"
            checked = self.acc_check.get(aid, True)
            self.acc_check[aid] = checked
            self.acc_tree.insert(
                "", "end", iid=aid,
                values=("☑" if checked else "☐", idx + 1, acc.get("name", aid),
                        acc.get("channel", "Kênh 1"), f"Nhóm {acc.get('login_position', 1)}", pid))
        # Drop ticks for deleted accounts.
        live = {str(a["id"]) for a in accounts}
        self.acc_check = {k: v for k, v in self.acc_check.items() if k in live}
        if self.selected_acc_id in live:
            self.acc_tree.selection_set(self.selected_acc_id)
    ...
    def _pg_auto_boss(self) -> ctk.CTkFrame:
        ...
        self.btn_boss_single = ctk.CTkButton(modes, text="Auto Boss (1 acc)", width=150,
                                              fg_color=C_ACCENT, hover_color=C_ACCENT_HOVER,
                                              command=self._toggle_boss)
        self.btn_boss_single.pack(padx=10, pady=(12, 6))
        self.btn_team = ctk.CTkButton(modes, text="Auto Boss (tự PT team)", width=150,
                                      command=self._toggle_team)
        self.btn_team.pack(padx=10, pady=6)
        ctk.CTkLabel(modes, text="Key", anchor="w").pack(padx=10, pady=(8, 0), anchor="w")
        self.team_key_menu = ctk.CTkOptionMenu(modes, values=self._account_names(), width=150)
        self.team_key_menu.pack(padx=10, pady=2)
        ctk.CTkLabel(modes, text="Member", anchor="w").pack(padx=10, pady=(6, 0), anchor="w")
        self.team_member_menu = ctk.CTkOptionMenu(modes, values=self._account_names(), width=150)
        self.team_member_menu.pack(padx=10, pady=2)
        self.lbl_team_state = ctk.CTkLabel(modes, text="PT: IDLE", text_color=C_TEXT_DIM,
                                           justify="left", wraplength=150)
        self.lbl_team_state.pack(padx=8, pady=8)
        # Preselect the configured key/member so the common case needs no input.
        try:
            team = self.cfg.load_team_config()
            names = {a["id"]: a["name"] for a in team.get("accounts", [])}
            if team.get("key_account") in names:
                self.team_key_menu.set(names[team["key_account"]])
            if team.get("members") and team["members"][0] in names:
                self.team_member_menu.set(names[team["members"][0]])
        except Exception:
            pass
        ...
        ctk.CTkButton(flash_row, text="Tìm boss", width=100, command=self._detect_boss_monsters).pack(side="left", padx=4)
        ...
        self.boss_monster_frame = ctk.CTkScrollableFrame(body, height=120, label_text="Danh sách boss trong map — tick để lưu boss cần đánh")
    ...
    def _toggle_train(self) -> None:
        trainer = getattr(self, "team_trainer", None)
        if trainer and trainer.state.running:
            trainer.stop()
            self.btn_train.configure(text="Train 5 acc (3 map)")
            return
        self.team_trainer = TeamTrainer(
            self.cfg, self.logger,
            evidence_path=Path(self.cfg.project_root) / "logs" / "team_train.json")
        self.team_trainer.start()
        self.btn_train.configure(text="Dung train")

    def _account_names(self) -> list[str]:
        names = [str(a.get("name") or a.get("id"))
                 for a in self.accounts_data.get("accounts", [])]
        return names or ["(no account)"]

    def _login_for_team(self, account_name: str) -> dict:
        """Return a logged-in client for this account, reusing an open one."""
        for win in self.wm.list_all():
            if (win.title or "").strip() == account_name and win.pid:
                detail = ""
                try:
                    ...
                    try:
                        ...
                        if player:
                            ...
                            detail = f'Player={hex(player["base"])} Core={hex(core)}'
                    finally:
                        mem.close()
                except Exception:
                    detail = ""
                if detail:
                    return {"ok": True, "pid": int(win.pid), "hwnd": int(win.hwnd or 0),
                            "detail": detail, "reused": True}
        account = next((a for a in self.accounts_data.get("accounts", [])
                        if str(a.get("name") or a.get("id")) == account_name), None)
        if not account:
            return {"ok": False, "detail": f"khong tim thay account {account_name}"}
        result = SingleAccountAutoStart(self.logger).start_account(
            self.cfg.load_global()["flash_exe"], account, 150)
        result["reused"] = False
        return result

    def _toggle_team(self) -> None:
        if getattr(self, "team_flow", None) and self.team_flow.state.running:
            self.team_flow.stop()
            self.btn_team.configure(text="Auto Boss (PT team)")
            return

        key_name = self.team_key_menu.get()
        member_name = self.team_member_menu.get()
        if key_name == member_name:
            messagebox.showwarning("PT team", "Key va member phai la hai account khac nhau.")
            return
        signatures = {sig for sig, var in self.boss_monster_vars.items() if var.get()}
        if not signatures:
            messagebox.showwarning("PT team", "Bam Detect Monsters va chon quai truoc.")
            return
        map_name = self.map_target_menu.get()

        self.btn_team.configure(text="Dung PT team")
        self.lbl_team_state.configure(text="PT: dang dang nhap...")

        def work() -> None:
            key = self._login_for_team(key_name)
            member = self._login_for_team(member_name)
            if not key.get("ok") or not member.get("ok"):
                failed = key_name if not key.get("ok") else member_name
                self.after(0, lambda: self.lbl_team_state.configure(
                    text=f"PT: login that bai ({failed})"))
                self.after(0, lambda: self.btn_team.configure(text="Auto Boss (PT team)"))
                return
            self.team_flow = TeamFlow(self.logger, ScreenCapture(), self.wm,
                                      str(Path(self.cfg.project_root) / "logs" / "team_panel_acc_2.png"))
            self.team_flow.start(key, member, map_name, signatures)

        threading.Thread(target=work, daemon=True).start()

    def _boss_account_and_map(self) -> tuple[str, int]:
        """Identify the bound client by account name and current map id.

        HWND and PID both change on relaunch, so neither can key a selection
        that must survive a reset. The window title carries the account name.
        """
        win = self.boss_flash_win_map.get(self.boss_selected_label)
        if not win:
            return "", 0
        account = (win.title or "").strip()
        try:
            map_id = MapTraveler.detect_map_memory(int(win.pid or 0))[1]
        except Exception:
            map_id = 0
        return account, int(map_id or 0)

    def _persist_boss_selection(self) -> None:
        """Write the ticked monsters to disk as soon as they are ticked."""
        account, map_id = self._boss_account_and_map()
        selected = {sig for sig, var in self.boss_monster_vars.items() if var.get()}
        win = self.boss_flash_win_map.get(self.boss_selected_label)
        if win:
            self.boss_selections[int(win.hwnd or 0)] = selected
        if not account or not map_id:
            return
        try:
            self.cfg.save_boss_selection(account, map_id, selected)
            self.lbl_boss_detail.configure(
                text=f'Saved {len(selected)} monster type(s) for {account} on map {map_id}')
        except Exception as exc:
            self.lbl_boss_detail.configure(text=f"Could not save monster selection: {exc}")
    ...
    def _render_boss_monsters(self, selected, monsters) -> None:
        ...
        for signature in signatures:
            ...
            ctk.CTkCheckBox(self.boss_monster_frame,
                            ...
                            variable=var,
                            command=self._persist_boss_selection).pack(anchor="w", padx=8, pady=3)
    ...
    def _finish_detect_monsters(self, win: WindowInfo, result: dict) -> None:
        ...
        if not selected:
            # Restore the choice saved for this account and map. This is what
            # makes the pick survive a Flash restart; the HWND-keyed cache above
            # is empty on a fresh PID.
            account = (win.title or "").strip()
            map_id = 0
            try:
                map_id = MapTraveler.detect_map_memory(int(win.pid or 0))[1]
                ...
                map_id = 0
            if account and map_id:
                saved = set(self.cfg.load_boss_selection(account, map_id))
                selected = saved & detected
    ...
    def _poll(self) -> None:
        self._poll_tick += 1
        flow = getattr(self, "team_flow", None)
        if flow is not None and hasattr(self, "lbl_team_state"):
            state = flow.state
            self.lbl_team_state.configure(
                text=(f"PT: {state.stage}\n{state.detail}\n"
                      f"nhom={'co' if state.party_ok else 'chua'} "
                      f"danh={state.attacks} clear={state.clears}"))
            if not state.running:
                self.btn_team.configure(text="Auto Boss (PT team)")
        trainer = getattr(self, "team_trainer", None)
        if trainer is not None and hasattr(self, "lbl_train_state"):
            st = trainer.state
            done = sum(sum(a.battles.values()) for a in st.accounts)
            self.lbl_train_state.configure(
                text=(f"Train: {st.stage}\n{st.detail}\n"
                      f"acc={len(st.accounts)} tran={done} "
                      f"restart={st.restarts}"))
            if not st.running:
                self.btn_train.configure(text="Train 5 acc (3 map)")
        if self._poll_tick % 5 == 0:
            self.team_binder.refresh(self.wm.list_all())
            self.team_binding_snapshot = self.team_binder.snapshot()
