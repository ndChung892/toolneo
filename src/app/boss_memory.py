...
class FlashMemory:
    ...
    _npc_view_cache: dict[tuple[int, int, int], int] = {}
    _npc_ref_region_cache: dict[int, list[tuple[int, int]]] = {}
    ...
    @classmethod
    def forget(cls, pid: int) -> None:
        """Drop everything cached about a pid.

        Both caches are keyed by pid at class level, and Windows reuses pids.
        A client that was closed and relaunched can therefore come back wearing
        the same pid while its AVM heap and entity vtable are completely
        different - the cached vtable would then point at nothing and every
        entity read would come back empty, or worse, read a stale address.
        Call this whenever a client is (re)bound rather than trusting that a
        pid means the same process it did five minutes ago.
        """
        cls._region_cache.pop(int(pid), None)
        cls._vtable_cache.pop(int(pid), None)
        cls._npc_ref_region_cache.pop(int(pid), None)
        for key in [key for key in cls._npc_view_cache if key[0] == int(pid)]:
            cls._npc_view_cache.pop(key, None)

    @classmethod
    def invalidate_map(cls, pid: int) -> None:
        """Drop map-lifetime data while preserving process-lifetime data."""
        cls._region_cache.pop(int(pid), None)
        cls._npc_ref_region_cache.pop(int(pid), None)
        for key in [key for key in cls._npc_view_cache if key[0] == int(pid)]:
            cls._npc_view_cache.pop(key, None)
    ...
    def find_npc_view(self, npc: dict, core: int) -> int:
        ...
        """
        npc_base = int(npc["base"])
        cache_key = (self.pid, npc_base, int(core))
        cached = self._npc_view_cache.get(cache_key, 0)
        if cached:
            method = self.method_at_slot(cached, 322)
            if (self.u32(cached + 0x118) & ~7) == npc_base \
                    and self.u32(cached + 0x198) == int(core) \
                    and method and method.get("method_id") and method.get("entry"):
                return cached
            self._npc_view_cache.pop(cache_key, None)

        needle = struct.pack("<I", npc_base | 1)
        matches: list[int] = []
        cached_regions = self._npc_ref_region_cache.get(self.pid, [])
        scan_regions = cached_regions or list(self.regions())
        hit_regions: list[tuple[int, int]] = []
        for base, size in scan_regions:
            ...
            while at >= 0:
                ...
                at = data.find(needle, at + 1)
        if not matches and cached_regions:
            hit_regions = []
            for base, size in self.regions():
                ...
                while at >= 0:
                    ...
                    at = data.find(needle, at + 1)
        if hit_regions:
            self._npc_ref_region_cache[self.pid] = list(dict.fromkeys(hit_regions))
        unique = list(dict.fromkeys(matches))
        if len(unique) == 1:
            self._npc_view_cache[cache_key] = unique[0]
            return unique[0]
        return 0
