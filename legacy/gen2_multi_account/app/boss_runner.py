...
from .boss_memory import FlashMemory, choose_player, monster_signatures, npc_candidates
from .window_manager import WindowInfo, WindowManager


CLICK_NPC_SLOT = 322
ACTIVE_VIEW_OFF = 0x320
AUTO_BOSS_BUILD = "MULTI_MONSTER_SMOOTH_V2_20260813"
MONSTER_NAMES = {
    (3060100000051, 2060100000051): ("Phat Tai", "verified"),
    (3060100001180, 2060100001208): ("Cai Nguc Ac Ma", "inferred"),
    (3060100001180, 2060100001211): ("Dau Si Ac Ma", "inferred"),
    (3060100001088, 2060100001088): ("Bach Thao Tinh", "inferred"),
    (3060100200016, 2060100200016): ("Nguoi Dua Tin", "inferred"),
}
...
class BossRunner:
    ...
    def set_selected_signatures(self, signatures) -> None:
        ...
        self.selected_signatures = {(int(a), int(b)) for a, b in signatures}
    ...
    def detect_monsters(self, window: WindowInfo | None = None,
        ...
        try:
            ...
            monsters = monster_signatures(rows)
            for item in monsters:
                name, confidence = MONSTER_NAMES.get(
                    (item["type"], item["resource"]),
                    (f'Monster {item["resource"]}', "fallback"))
                item["name"], item["name_confidence"] = name, confidence
            self._monster_cache[int(win.pid)] = (time.monotonic(), monsters)
            return {"ok": True, "pid": win.pid, "monsters": monsters, "cached": False}
        ...
        while self.state.running and generation == self._run_generation:
            ...
            try:
                ...
                if not self.target:
                    ...
                    self.logger.info(
                        "AUTO_BOSS",
                        f'selected monster id={self.target["runtime_id"]} '
