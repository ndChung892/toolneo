...
class App(ctk.CTk):
    ...
    def _pg_auto_boss(self) -> ctk.CTkFrame:
        ...
        self.boss_flash_menu = ctk.CTkOptionMenu(
            controls, values=["(Refresh Flash)"], width=240,
            command=self._select_boss_flash,
        )
        self.boss_flash_menu.pack(side="left", padx=(10, 4), pady=10)
        ctk.CTkButton(controls, text="Refresh", width=72, command=self._refresh_boss_flashes).pack(side="left", padx=4)
        ctk.CTkButton(controls, text="Detect Monsters", width=125, command=self._detect_boss_monsters).pack(side="left", padx=4)
    ...
    def _finish_detect_monsters(self, win: WindowInfo, result: dict) -> None:
        ...
        if not result.get("ok"):
            ...
            return
        detected = {(m["type"], m["resource"]) for m in result["monsters"]}
