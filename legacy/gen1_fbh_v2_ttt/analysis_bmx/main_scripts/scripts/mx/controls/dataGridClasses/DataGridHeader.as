package mx.controls.dataGridClasses
{
   ...
   public class DataGridHeader extends DataGridHeaderBase
   {
      ...
      protected function mouseUpHandler(param1:MouseEvent) : void
      {
         ...
         {
            _loc5_ = int(visibleColumns.length);
            _loc6_ = 0;
            while(_loc6_ < _loc5_)
            {
         ...
         if(Boolean(dataGrid.enabled && (dataGrid.sortableColumns || dataGrid.draggableColumns)) && Boolean(dataGrid.dataProvider) && dataGrid.headerVisible)
         {
            if(_loc3_ == lastItemDown)
            {
               if(dataGrid.sortableColumns && Boolean(visibleColumns[_loc6_].sortable))
               {
                  lastItemDown = null;
      ...
      protected function drawSeparators() : void
      {
         ...
         var _loc2_:* = visibleColumns ? int(visibleColumns.length) : 0;
      ...
      protected function mouseOutHandler(param1:MouseEvent) : void
      {
         ...
         if(dataGrid.enabled && dataGrid.sortableColumns && dataGrid.headerVisible)
         {
            ...
            {
               _loc4_ = int(visibleColumns.length);
               _loc3_ = 0;
               while(_loc3_ < _loc4_)
               {
      ...
      private function columnDraggingMouseUpHandler(param1:MouseEvent) : void
      {
         ...
         dataGrid.shiftColumns(_loc2_,dropColumnIndex,param1);
         ...
         var _loc4_:Sprite = Sprite(dataGrid.getChildByName("columnDragSelectionLayer"));
         if(!_loc4_)
         {
            startX = NaN;
         ...
         var _loc5_:IListItemRenderer = IListItemRenderer(getChildByName("headerDragProxy"));
         if(_loc5_)
         {
            _loc4_.removeChild(DisplayObject(_loc5_));
         }
         var _loc6_:Sprite = Sprite(_loc4_.getChildByName("headerSelection"));
         if(_loc6_)
         {
         ...
         _loc6_ = Sprite(dataGrid.getChildByName("columnDragOverlay"));
         if(_loc6_)
         {
            dataGrid.removeChild(_loc6_);
         }
         dataGrid.removeChild(_loc4_);
         startX = NaN;
         dataGrid.movingColumn = null;
         dropColumnIndex = -1;
      ...
      private function columnDraggingMouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         if(isNaN(startX))
         {
            ...
            _loc14_.setActualSize(_loc3_.width,dataGrid._explicitHeaderHeight ? dataGrid.headerHeight : Number(_loc14_.getExplicitOrMeasuredHeight()));
            _loc5_ = 0;
            while(_loc5_ < _loc6_)
            {
      ...
      protected function placeSortArrow() : void
      {
         ...
         var _loc6_:Boolean = dataGrid.sortDirection == "ASC";
         sortArrow.width = sortArrow.measuredWidth;
         sortArrow.height = sortArrow.measuredHeight;
         DisplayObject(sortArrow).scaleY = _loc6_ ? -1 : 1;
      ...
      private function columnResizeMouseUpHandler(param1:MouseEvent) : void
      {
         if(!enabled || !dataGrid.resizableColumns)
         {
            return;
         ...
         resizeGraphic = null;
         cursorManager.removeCursor(resizeCursorID);
         var _loc2_:DataGridColumn = resizingColumn;
         resizingColumn = null;
         var _loc3_:int = dataGrid.vScrollBar ? int(dataGrid.vScrollBar.width) : 0;
         var _loc4_:Point = new Point(param1.stageX,param1.stageY);
         _loc4_ = dataGrid.globalToLocal(_loc4_);
         var _loc5_:Number = Math.min(Math.max(minX,_loc4_.x),dataGrid.width / dataGrid.scaleX - separators[0].width - _loc3_) - startX;
         dataGrid.resizeColumn(_loc2_.colNum,Math.floor(_loc2_.width + _loc5_));
         invalidateDisplayList();
         var _loc6_:DataGridEvent = new DataGridEvent(DataGridEvent.COLUMN_STRETCH);
         _loc6_.columnIndex = _loc2_.colNum;
         _loc6_.dataField = _loc2_.dataField;
         _loc6_.localX = _loc4_.x;
         dataGrid.dispatchEvent(_loc6_);
      }
      ...
      override protected function measure() : void
      {
         ...
         cachedHeaderHeight = dataGrid._explicitHeaderHeight ? dataGrid.headerHeight : _loc1_;
      ...
      protected function mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         {
            if(dataGrid.sortableColumns && Boolean(visibleColumns[_loc4_].sortable))
            {
               lastItemDown = _loc2_;
            ...
            if(dataGrid.draggableColumns && Boolean(visibleColumns[_loc4_].draggable))
            {
               startX = NaN;
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(headerItemsChanged && (Boolean(_loc3_ && _loc3_.length > 0) || Boolean(dataGrid.headerVisible)))
         {
            ...
            while(Boolean(_loc3_) && _loc10_ < _loc3_.length)
            {
               ...
               _loc12_.setActualSize(_loc8_,dataGrid._explicitHeaderHeight ? cachedHeaderHeight - cachedPaddingTop - cachedPaddingBottom : _loc9_);
               _loc12_.move(_loc5_,_loc6_ + cachedPaddingTop);
               _loc5_ += _loc8_;
               _loc10_++;
               _loc7_ = Math.ceil(Math.max(_loc7_,dataGrid._explicitHeaderHeight ? cachedHeaderHeight : _loc9_ + cachedPaddingBottom + cachedPaddingTop));
               _loc17_ = Math.max(_loc17_,dataGrid._explicitHeaderHeight ? cachedHeaderHeight - cachedPaddingTop - cachedPaddingBottom : _loc9_);
            }
            _loc18_ = 0;
            while(_loc18_ < headerItems.length)
            {
               headerItems[_loc18_].setActualSize(headerItems[_loc18_].width,_loc17_);
               _loc18_++;
            }
