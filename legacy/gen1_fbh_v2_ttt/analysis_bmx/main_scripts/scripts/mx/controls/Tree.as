package mx.controls
{
   ...
   public class Tree extends List implements IIMESupport
   {
      ...
      public function Tree()
      {
         ...
         addEventListener(TreeEvent.ITEM_OPENING,mx_internal::expandItemHandler,false,EventPriority.DEFAULT_HANDLER);
      }
      ...
      private function updateDropData(param1:DragEvent) : void
      {
         ...
         else if(!_loc13_ && !_loc3_ == _loc2_)
         {
            ...
            _loc10_ = _loc14_ ? getChildIndexInParent(_loc9_,_loc14_) : 0;
            _loc3_ = 0;
         }
         ...
         else if(Boolean(_loc13_) && Boolean(_loc14_) && _loc15_ == _loc16_)
         {
            ...
            _loc10_ = iterator ? getChildIndexInParent(_loc9_,_loc14_) : 0;
         }
            ...
            if(depthColors)
            {
               try
               {
                  if(listItems[curRow][0])
                  {
                     ...
                     rowColor = colors[d - 1] ? uint(colors[d - 1]) : uint(color);
      ...
      private function getVisibleChildrenCount(param1:Object) : int
      {
         ...
         if(Boolean(_openItems[_loc3_]) && Boolean(_dataDescriptor.isBranch(param1,iterator.view)) && _dataDescriptor.hasChildren(param1,iterator.view))
         {
         ...
         if(_loc4_ != null)
         {
            ...
            while(!_loc5_.afterLast)
            {
               ...
               {
                  _loc2_ += getVisibleChildrenCount(_loc5_.current);
               }
               _loc5_.moveNext();
            }
         }
      ...
      {
         return _dataDescriptor.getChildren(param1,param2);
      }
      ...
      override public function itemToIcon(param1:Object) : Class
      {
         ...
         uid = itemToUID(item);
         iconClass = Boolean(itemIcons) && Boolean(itemIcons[uid]) ? itemIcons[uid][open ? "iconID2" : "iconID"] : null;
         if(iconClass)
         {
            return iconClass;
         }
      ...
      public function expandItem(param1:Object, param2:Boolean, param3:Boolean = false, param4:Boolean = false, param5:Event = null) : void
      {
         ...
         event = new CollectionEvent(CollectionEvent.COLLECTION_CHANGE,false,true,CollectionEventKind.EXPAND);
         event.items = [item];
         if(open)
         {
            _openItems[uid] = item;
            ...
            if(animate && rowIndex < rC && rowsToMove > 0 && rowsToMove < 20 && dur != 0)
            {
               ...
               else
               {
                  ...
                  while(i < rowsToMove && yy < maxDist)
                  {
                     ...
                     {
                        hh = rowIndex + i > 0 ? Number(rowInfo[rowIndex + i - 1].height) : rowHeight;
                     }
                     else if(rowList[i])
                     {
