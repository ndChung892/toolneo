package mx.controls
{
   ...
   public class ProgressBar extends UIComponent implements IFontContextComponent
   {
      ...
      override protected function measure() : void
      {
         ...
         var _loc6_:Number = _track.measuredWidth;
         var _loc7_:Number = isNaN(_loc5_) ? _track.measuredHeight : _loc5_;
         ...
         var _loc16_:Number = isNaN(_loc14_) ? _loc15_.width + UITextField.TEXT_WIDTH_PADDING : _loc14_;
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         _loc12_ = isNaN(_loc12_) ? _track.measuredHeight : _loc12_;
         var _loc13_:TextLineMetrics = measureText(predictLabelText());
         var _loc14_:Number = isNaN(_loc11_) ? _loc13_.width + UITextField.TEXT_WIDTH_PADDING : _loc11_;
         var _loc15_:Number = _loc13_.height + UITextField.TEXT_HEIGHT_PADDING;
         switch(labelPlacement)
         {
            case ProgressBarLabelPlacement.TOP:
               _labelField.move(_loc9_,_loc10_);
               _labelField.setActualSize(_loc14_,_loc15_);
               _content.move(_loc9_,_loc10_ + _loc15_ + _loc4_);
               layoutContent(param1 - _loc9_ - _loc6_,_loc12_);
               break;
            case ProgressBarLabelPlacement.RIGHT:
               _loc16_ = param1 - _loc9_ - _loc6_ - _loc14_ - _loc3_;
               _labelField.move(_loc9_ + _loc16_ + _loc3_,(param2 - _loc15_) / 2);
               _labelField.setActualSize(_loc14_,_loc15_);
               _content.move(_loc9_,_loc10_ + (_loc15_ - _loc12_) / 2);
               layoutContent(_loc16_,_loc12_);
               break;
            case ProgressBarLabelPlacement.LEFT:
               _labelField.move(_loc9_,_loc10_ + (param2 - _loc15_) / 2);
               _labelField.setActualSize(_loc14_,_loc15_);
               _content.move(_loc9_ + _loc14_ + _loc3_,_loc10_ + (_loc15_ - _loc12_) / 2);
               layoutContent(param1 - _loc9_ - _loc14_ - _loc4_ - _loc6_,_loc12_);
               break;
            case ProgressBarLabelPlacement.CENTER:
               _labelField.move((param1 - _loc14_) / 2,(param2 - _loc15_) / 2);
               _labelField.setActualSize(_loc14_,_loc15_);
               _content.move(_loc9_,_loc10_);
               layoutContent(param1 - _loc6_,param2 - _loc8_);
               break;
            default:
               _labelField.move(_loc9_,_loc10_ + _loc12_ + _loc4_);
               _labelField.setActualSize(_loc14_,_loc15_);
               _content.move(_loc9_,_loc10_);
               layoutContent(param1 - _loc9_ - _loc6_,_loc12_);
         }
         if(_barMask)
         {
            ...
            {
               _loc17_ = UIComponent(_barMask).graphics;
               _loc17_.clear();
               _loc17_.beginFill(16776960);
               _loc17_.drawRect(1,1,_track.width - 2,_track.height - 2);
               _loc17_.endFill();
            }
         }
         ...
         {
            _loc18_ = Math.max(0,_track.width * percentComplete / 100);
            _determinateBar.setActualSize(_loc18_,_track.height);
            _determinateBar.x = direction == ProgressBarDirection.RIGHT ? 0 : _track.width - _loc18_;
         }
      }
