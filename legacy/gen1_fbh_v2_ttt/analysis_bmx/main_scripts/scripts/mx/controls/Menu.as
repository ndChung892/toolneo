package mx.controls
{
   ...
   public class Menu extends List implements IFocusManagerContainer
   {
      ...
      override public function measureWidthOfItems(param1:int = -1, param2:int = 0) : Number
      {
         ...
         return _loc3_ + (getStyle("paddingLeft") + getStyle("paddingRight"));
      }
      ...
      mx_internal function openSubMenu(param1:IListItemRenderer) : void
      {
         ...
         if(!IMenuItemRenderer(param1).menu)
         {
            ...
            if(Boolean(param1.data) && Boolean(_dataDescriptor.isBranch(param1.data)) && _dataDescriptor.hasChildren(param1.data))
            {
         ...
         _loc3_.show(_loc5_.x + param1.width,_loc5_.y);
         subMenu = _loc3_;
      ...
      public function hide() : void
      {
         ...
         if(visible)
         {
            ...
            visible = false;
            systemManager.removeEventListener(MouseEvent.MOUSE_DOWN,mouseDownOutsideHandler);
            _loc1_ = new MenuEvent(MenuEvent.MENU_HIDE);
            _loc1_.menu = this;
            _loc1_.menuBar = sourceMenuBar;
            getRootMenu().dispatchEvent(_loc1_);
         }
      }
      ...
      override protected function keyDownHandler(param1:KeyboardEvent) : void
      {
         ...
         {
            if(Boolean(_loc3_ && _dataDescriptor.isBranch(_loc3_)) && Boolean(_loc4_) && _loc4_.visible)
            {
               supposedToLoseFocus = true;
               _loc4_.setFocus();
         ...
         {
            if(Boolean(_loc3_) && _dataDescriptor.isBranch(_loc3_))
            {
               openSubMenu(_loc2_);
      ...
      {
         if(!isMouseOverMenu(param1) && !isMouseOverMenuBarItem(param1))
         {
            hideAllMenus();
         }
      }
      ...
      override protected function commitProperties() : void
      {
         ...
         if(dataProviderChanged || showRootChanged)
         {
            ...
            showRootChanged = false;
            if(Boolean(_rootModel) && Boolean(!_showRoot) && _hasRoot)
            {
      ...
      override protected function mouseOverHandler(param1:MouseEvent) : void
      {
         ...
         row = mouseEventToItemRenderer(event);
         if(!row)
         {
            return;
         }
         ...
         if(Boolean(item) && _dataDescriptor.isEnabled(item))
         {
      ...
      public function show(param1:Object = null, param2:Object = null) : void
      {
         ...
         if(Boolean(parentDisplayObject) && parent != parentDisplayObject)
         {
         ...
         if(this != getRootMenu())
         {
            _loc5_ = x + width - screen.width;
            if(_loc5_ > 0)
            {
               x = Math.max(x - _loc5_,0);
            }
            _loc5_ = y + height - screen.height;
            if(_loc5_ > 0)
            {
               y = Math.max(y - _loc5_,0);
            }
         }
         UIComponentGlobals.layoutManager.validateClient(this,true);
         setActualSize(getExplicitOrMeasuredWidth(),getExplicitOrMeasuredHeight());
         cacheAsBitmap = true;
         var _loc4_:Number = getStyle("openDuration");
         if(_loc4_ != 0)
         {
            ...
            popupTween = new Tween(this,[0,0],[unscaledWidth,unscaledHeight],_loc4_);
         }
         else
         {
            UIComponentGlobals.layoutManager.validateNow();
