   ...
   public class Panel extends Container implements IConstraintLayout, IFontContextComponent
   {
      ...
      override protected function createChildren() : void
      {
         ...
         if(!titleBarBackground)
         {
            ...
            if(_loc1_)
            {
               titleBarBackground = new _loc1_();
               _loc2_ = titleBarBackground as IStyleClient;
               if(_loc2_)
               {
                  _loc2_.setStyle("backgroundImage",undefined);
               }
               _loc3_ = titleBarBackground as ISimpleStyleClient;
               if(_loc3_)
               {
      ...
      override protected function layoutChrome(param1:Number, param2:Number) : void
      {
         ...
         if(getQualifiedClassName(border) == "mx.skins.halo::PanelSkin" && getStyle("borderStyle") != "default" && Boolean(_loc4_))
         {
            _loc3_ = new EdgeMetrics(_loc4_,_loc4_,_loc4_,_loc4_);
         }
         var _loc5_:EdgeMetrics = FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0 ? borderMetrics : _loc3_;
         var _loc6_:Number = _loc5_.left;
         var _loc7_:Number = _loc5_.top;
         var _loc8_:Number = getHeaderHeight();
         if(_loc8_ > 0 && height >= _loc8_)
         {
            ...
            IFlexDisplayObject(titleBarBackground).setActualSize(_loc9_,_loc8_);
            ...
            if(_showCloseButton)
            {
               ...
               closeButton.move(param1 - _loc6_ - _loc5_.right - 10 - closeButton.getExplicitOrMeasuredWidth(),(_loc8_ - closeButton.getExplicitOrMeasuredHeight()) / 2);
            }
            _loc11_ = 10;
            _loc12_ = 10;
            if(titleIconObject)
            {
               _loc13_ = Number(titleIconObject.height);
               _loc14_ = (_loc8_ - _loc13_) / 2;
               titleIconObject.move(_loc11_,_loc14_);
               _loc11_ += titleIconObject.width + 4;
            }
            ...
            titleTextField.setActualSize(Math.max(0,param1 - _loc11_ - _loc12_ - _loc15_),_loc13_ + UITextField.TEXT_HEIGHT_PADDING);
            if(FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0)
            {
            ...
            if(_showCloseButton)
            {
               _loc16_ -= closeButton.getExplicitOrMeasuredWidth() + 4;
            }
      ...
      override protected function measure() : void
      {
         ...
         if(titleIconObject)
         {
            _loc2_ += titleIconObject.width;
         }
         if(closeButton)
         {
            _loc2_ += closeButton.getExplicitOrMeasuredWidth() + 6;
         }
      ...
      private function measureHeaderText() : Rectangle
      {
         ...
         if(Boolean(titleTextField) && Boolean(titleTextField.text))
         {
            ...
            _loc4_ = _loc3_.measureText(titleTextField.text,false);
            _loc1_ = _loc4_.width;
      ...
      private function showTitleBar(param1:Boolean) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            _loc4_ = titleBar.getChildAt(_loc3_);
         ...
         if(_loc2_ || param1 == "titleBackgroundSkin")
         {
            if(titleBar)
            {
               ...
               if(_loc6_)
               {
                  ...
                  titleBarBackground = new _loc6_();
                  _loc7_ = titleBarBackground as IStyleClient;
                  if(_loc7_)
                  {
                     _loc7_.setStyle("backgroundImage",undefined);
                  }
                  _loc8_ = titleBarBackground as ISimpleStyleClient;
                  if(_loc8_)
                  {
