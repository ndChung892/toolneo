...
class ClickController:
    ...
    def mouse_down_client(self, window: WindowInfo, cx: int, cy: int) -> bool:
        if not _WIN32_AVAILABLE:
            return False
        hwnd = self._resolve_hwnd(window)
        if not hwnd:
            return False
        target, tx, ty = self._resolve_click_target(hwnd, int(cx), int(cy))
        lp = _makelparam(tx, ty)
        try:
            win32api.SendMessage(target, win32con.WM_MOUSEMOVE, 0, lp)
            win32api.SendMessage(target, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, lp)
            return True
        except Exception:
            return False

    def mouse_up_client(self, window: WindowInfo, cx: int, cy: int) -> bool:
        if not _WIN32_AVAILABLE:
            return False
        hwnd = self._resolve_hwnd(window)
        if not hwnd:
        ...
        target, tx, ty = self._resolve_click_target(hwnd, int(cx), int(cy))
        try:
            win32api.SendMessage(target, win32con.WM_LBUTTONUP, 0, _makelparam(tx, ty))
            return True
        except Exception:
            return False
